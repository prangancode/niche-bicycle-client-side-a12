...
const Dashboard = () => {
    ...
    useEffect(() => {
        fetch(`https://pacific-ravine-90140.herokuapp.com/checkAdmin/${user?.email}`)
            .then((res) => res.json())
            .then((data) => {
                ...
                <aside className="w-1/5 h-auto bg-gray shadow-md hidden sm:block">
                    <div className="flex flex-col justify-between h-auto sm:p-0 lg:p-4 bg-gray-800">
                        <div className="text-sm lg:flex lg:flex-col">
                            {user.email && <div className="bg-gray-900 text-white p-3 rounded w-full mt-4 sm:text-md lg:text-2xl cursor-pointer hover:bg-gray-700 hover:text-blue-300"> {user.displayName}
                            </div>}
