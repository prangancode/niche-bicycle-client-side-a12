...
const ManageAllOrders = () => {
    ...
    useEffect(() => {
        fetch('https://pacific-ravine-90140.herokuapp.com/cycleBookings')
            .then(res => res.json())
    ...
    const handleCancelBooking = (id) => {
        ...
        console.log(id);
        if (proceed) {
            fetch(`https://pacific-ravine-90140.herokuapp.com/cycleBookings/${id}`, {
                method: 'DELETE',
                headers: {
    ...
    return (
        <div className="">
            <h1 className='font-mono m-5 italic underline sm:text-3xl lg:text-5xl text-center text-purple-600'>Manage All Orders</h1>
                ...
                <div className=" wrapper sm:px-0 sm:py-0 px-3 py-4 flex justify-center">
                    <table className=" text-md bg-white shadow-md rounded mb-4">
                        <tbody>
                            ...
                            {allOrders.map(order =>
                            (<tr className="border-b hover:bg-orange-100 bg-purple-200">
